        ...
        for block_num, block in enumerate(blocks):
            if re.search(self.RE_FENCE_END, block):
                ...
                for i in range(0, block_num + 1):
                    ...
                    # Rejoin with a single empty string so the eventual
                    # `\n`-join produces one blank line per split point, not
                    # the four-newline pile-up that `"\n\n"` here used to
                    # produce.
                    code_text.append("")
                    blocks.pop(0)

                code_text = code_text[:-1]
                # Escape literal backticks inside non-deduce code blocks so the
                # inline `code` pattern (which scans element text after block
                # processing) doesn't sweep up backticks in the code itself —
                # e.g. elisp's `\`C-c C-a'\`` quoting style, which has unmatched
                # singles that the inline regex would pair across lines.
                code.text = '<!-- Generated by codeUtils.js -->' if m else safeHTMLify('\n'.join(code_text)).replace("`", "&#96;")
                if m: 
