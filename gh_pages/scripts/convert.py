...
mdToHtmlName = {
    ...
    'Reference' : 'reference',
    'ImperativeReference' : 'imperative-reference',
...
mdToTitle = {
    ...
    'ImperativeReference' : 'Imperative Reference (experimental)',
...
mdToDescription = {
    ...
    'ImperativeReference' : 'Reference for Deduce\'s experimental imperative surface (proc, observer, object, mutable arrays, frame expressions).',
...
class BlockPreprocessor(Preprocessor):  # type: ignore[misc]
    ...
    def is_list(self, line):
        ...
        # `**bold**` would be matched as a `*`-bullet and the next line would
        # be split off as a separate paragraph by the transition logic below.
        list_start = [r'\*\s', r'-\s', r'\+\s', r'[0-9]+\.\s']
    ...
    def run(self, lines):
        ...
        for line in lines:
            ...
            # Insert a blank line at list boundaries so the markdown parser
            # sees the list as its own block. Exception: when leaving a list,
            # don't add a blank line if the next line is an indented
            # continuation of the previous list item — otherwise markdown
            # closes the item early and renders the continuation as a
            # standalone paragraph.
            if cur_is_list and not prev_is_list:
                new_lines.append("")
            elif prev_is_list and not cur_is_list and not self.is_indented(line):
                new_lines.append("")
        ...
        for block_num, block in enumerate(blocks):
            if re.search(self.RE_FENCE_END, block):
                ...
                for i in range(0, block_num + 1):
                    ...
                    # Markdown split the original code fence into separate
                    # blocks wherever a whitespace-only line lived inside it.
                    # Carry one blank line per split point in the separator
                    # element so the subsequent `''.join` reproduces exactly
                    # the original spacing both in the rendered HTML (via
                    # `safeHTMLify` below) and in the entangled `.pf` output
                    # (via `''.join(code_text)` further down).
                    code_text.append("\n\n")
                    blocks.pop(0)

                code_text = code_text[:-1]
                # Escape literal backticks and backslashes inside non-deduce
                # code blocks so the inline pass (which scans element text
                # after block processing) doesn't rewrite them:
                #   * backticks — the inline `code` pattern would pair an
                #     unmatched opening backtick across lines (e.g. elisp's
                #     `\`C-c C-a'\`` quoting style);
                #   * backslashes — the inline ESCAPE pattern converts
                #     `\X` into an STX/ETX-wrapped codepoint placeholder
                #     that the unescape tree processor refuses to restore
                #     inside `<code>` text, leaving the raw codepoint
                #     digits visible (e.g. `\\` → `92`).
                code.text = '<!-- Generated by codeUtils.js -->' if m else safeHTMLify(''.join(code_text)).replace("`", "&#96;").replace("\\", "&#92;")
                if m: 
