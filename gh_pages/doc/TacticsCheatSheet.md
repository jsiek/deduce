...
11. **`switch n` on `UInt` accepts the public view constructors.** Even though `UInt`'s underlying constructors (`bzero`, `dub_inc`, `inc_dub`) are private, `switch n { case zero { ... } case suc(m) { ... } }` is accepted directly and behaves like a `Nat`-style `switch`: each case body sees the goal with `n` substituted by `0` or `1 + m`. Caveats: the subject must be a variable (use `define n = <expr>` first otherwise); the case-style equation hypothesis (`assume eq: n = 0`) is not bound on this path — use `cases uint_zero_or_add_one[n]` (`all x:UInt. x = 0 or (some x':UInt. x = 1 + x')`) when you need the equation as a labelled fact. The IH from the supporting induction theorem is auto-suppressed; use `induction UInt` if you actually want it.

12. **`induction UInt` uses bijective-view constructors.** Write `case zero { ... }` / `case suc(m) assume IH { ... }` when UInt's inverse-backed view is in scope; the registered `uint_induction` theorem supplies the recursive IH behind that public view syntax.
