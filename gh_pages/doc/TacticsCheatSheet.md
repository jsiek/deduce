...
11. **`switch` on `UInt` rejects the public 0 / 1+m view.** `UInt`'s constructors (`bzero`, `dub_inc`, `inc_dub`) are private, so `switch n { case 0 { ... } case with m. 1 + m { ... } }` fails with a redirect to `induction UInt`. For *inductive* case-splits, use `induction UInt` — the IH on the `1 + m` branch can simply be ignored if unused. For *non-recursive* case-splits, use the library lemma `uint_zero_or_add_one : all x:UInt. x = 0 or (some x':UInt. x = 1 + x')` together with `cases uint_zero_or_add_one[n]`.
