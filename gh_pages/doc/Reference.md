...
`reads` and `modifies` clauses of a [Procedure](#procedure-statement) or
the `reads` clause of an [Observer](#observer-statement). The empty set
is written `{}`, `footprint(e)` names the ghost footprint owned by `e`,
`e.f` names one mutable object field, and a bare term `e` names every
cell of a mutable structure. A frame list is a comma-separated union.

Frame expressions are parser-only in Phase 1 of the imperative
verification plan (issue #854); the verifier that consumes them has not
...
`opaque object` hides its fields outside the defining module; otherwise
fields follow the object's visibility.

Object declarations are parsed and uniquified today, but the supporting
imperative verifier from issue #854 is not implemented yet, so
mutation, allocation, and field reads/writes have no runtime or proof
semantics.
...
Observer declarations require the `--experimental-imperative` flag.
They are parser-only in Phase 1 of issue #854; the checker rejects
them until the imperative verifier exists.
...
Procedure declarations require the `--experimental-imperative` flag.
They are parser-only in Phase 1 of issue #854; only an empty body is
accepted today and the checker rejects every procedure until the
imperative verifier exists. Frame clauses use the
