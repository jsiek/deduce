...
Counters such as ``proof_checker.name_id`` are intentionally **not**
restored: letting them increase monotonically guarantees that names
generated by a later call never collide with cached prelude names.
"""
...
def check_file(
    ...
    return _check_file_impl(filename, tracing_functions, prelude, content)
...
# state we capture once and restore between calls. Counters (e.g.
# ``proof_checker.name_id``) are intentionally absent -- they need to
# increase monotonically across calls so freshly generated names never
# collide with cached prelude names. If a future change adds a new
# container that contributes to per-call results, it goes here.
...
def _prepare_state(prelude_key: tuple[str, ...]) -> None:
    ...
    """
    global _prelude_key, _prelude_snapshot
