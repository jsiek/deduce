...
- **Concurrent fill-hole requests in the same buffer.** Two `?`s, two requests fired in parallel: each gets its own marker pair; the second's range may shift if the first applies first. Marker + fingerprint checks catch that and reject the stale result without editing the buffer. Auto-retry-with-refreshed-context is still a follow-up.
