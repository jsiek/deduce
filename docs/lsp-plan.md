...
- [x] **Step 31: `textDocument/completion` — in-buffer tab completion.** New `completions_at(path, content, pos, prelude)` in `lsp/query.py` returns a tuple of `CompletionCandidate(label, kind, detail)` items: Deduce keywords (static list mirroring `deduce-mode--keywords`), constants (`true`/`false`), built-in types (`bool`/`type`), and every top-level name reachable from the file — its own theorems / lemmas / postulates / defines / functions / unions / constructors / predicates / rules, plus the transitive walk through `Import.ast` (so `import Nat` surfaces `suc`, `zero`, `+`, `length`, ...).  Names reachable through multiple imports collapse to one entry.  `lsp/lsp_server.py` wires `TEXT_DOCUMENT_COMPLETION` via `pygls.feature`, mapping `kind` strings to LSP `CompletionItemKind` values (`keyword`→Keyword, `constant`→Constant, `type`→Class, `theorem`/`lemma`/`function`/`postulate`/`predicate`/`rule`→Function, `define`→Variable, `union`→Class, `constructor`→Constructor); returns `CompletionList(is_incomplete=False, ...)` so the client can cache and filter locally.  10-case pytest in `test/lsp/test_completion.py` locks the candidate set across own-file / transitive-import / parse-failure shapes.  Editor wiring: both eglot and `vscode-languageclient` auto-register a completion handler as soon as the server advertises `completionProvider`; `M-TAB` / `C-M-i` in Emacs and `Ctrl+Space` in VS Code work out of the box.  Local bindings introduced by `arbitrary` / `assume` / `obtain` are **not** yet surfaced — needs an env walker for arbitrary cursor positions, tracked as a Step-31 follow-up.
