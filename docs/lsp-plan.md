...
- [x] **Step 26: Editor packages.** Wire DAP into both editors:
  ...
  - *Implementation:* **Emacs** — ``editor/emacs/deduce-dap.el`` registers a ``dap-mode`` provider + template for ``type "deduce"``; mirrors ``deduce-lsp``'s customisation surface (``deduce-dap-python-program``, ``deduce-dap-deduce-root``, ``deduce-dap-prelude-disabled``).  ``C-c C-d`` in any ``.pf`` buffer launches via ``dap-debug``.  9-case ert suite in ``editor/emacs/test/deduce-dap-test.el`` pins the command shape under each customisation knob and confirms the keybinding is installed.  Manual smoke script appended to ``editor/emacs/README.md`` (steps 14–19).  **VS Code** — ``editor/vscode/`` is now the canonical home for the Deduce VS Code extension (the previous out-of-tree [HalflingHelper/deduce-mode](https://github.com/HalflingHelper/deduce-mode) extension is no longer maintained).  Ships a minimal declarative ``package.json`` registering the ``deduce`` debugger type and language, plus a ``launch.json.sample`` template and a step-by-step README covering both standalone (``code --extensionDevelopmentPath``) and packaged-install workflows (``vsce package`` -> ``code --install-extension``).  Syntax highlighting and LSP-client wiring are tracked as follow-up work in ``editor/vscode/README.md``'s roadmap.  The user-facing ``gh_pages/doc/Debugger.md`` gains a refreshed *Editor integration* section pointing at both packages.
