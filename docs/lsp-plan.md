...
**Status:** Phase 1, Phase 2 Step 9, Phase 3 Steps 12–14, Phase 4 Steps 15–19, Phase 5 Steps 21–23 + 25–26, and Phase 6 Steps 27–32 are done.  Open: Steps 10 (process-lifecycle hardening), 11 (multi-error collection), 24 (tactic stepping), 33 (marketplace publication).
...
- [x] **Step 3: Implement `check`.** Convert raised `Exception` / `StaticError` / `IncompleteProof` into `Diagnostic` objects. Single-diagnostic mode is fine; multi-error collection is Step 10.
  ...
  - *Implementation:* `lsp/query.py:check` calls `check_file(path, content=content)` and translates the captured exception into a `Diagnostic`. Required surfacing structured location/body via `error.py` (added `location`/`message_body` attributes to `UserError`/`IncompleteProof`/`StaticError`/`MatchFailed`/`ParseError`), a new `wrap_user_error` helper, and patches at six wrap sites in `proof_checker.py` that previously raised bare `Exception(str(e) + ...)` and dropped the location. `lsp/library.py` gained a `content` parameter (bypasses the uniquified-modules cache) and an `exception` field on `CheckResult`. `rec_desc_parser.py` had a pre-existing leak: `check_closest_kwd` was set to `True` on first parse error and never reset, leaking spurious "did you mean" suggestions across calls in library mode — now reset in a `finally`. Acceptance test `test/lsp/test_check.py` runs 158 cases.
...
2. **`abstract_syntax.terms:do_function_call`** — before/after every user-function reduction. The single layer that catches both `Lambda` and `RecFun` calls; PR #269's fix (also hooking `Call.reduce`'s `Lambda` arm and `do_recursive_call`'s case dispatch) double-fires and should not be carried over.
3. **`abstract_syntax.terms:Call.reduce`** — only for `Lambda` literals applied directly. Lambdas don't go through `do_function_call` because they have no name binding; they need their own one-line hook.
...
- [x] **Step 30: Structured-editing commands in VS Code.** Five commands that mirror the Emacs `C-c C-{r,c,i,e,f}` bindings: `deduce.refineHole` (Ctrl+Alt+R), `deduce.induction` (Ctrl+Alt+I), `deduce.caseSplit` (Ctrl+Alt+C), `deduce.eliminate` (Ctrl+Alt+E), `deduce.fillFromGiven` (Ctrl+Alt+F).  `refineHole` and `induction` issue `textDocument/codeAction` and apply the action whose title matches "Refine hole" / "Induction" (the Step-15 / Step-17 outputs) directly; `caseSplit` and `eliminate` first issue `deduce/{splittableVars,eliminableVars}At` to populate a `vscode.window.showQuickPick` of candidates, then issue the matching custom edit request with the chosen `variable` / `label`; `fillFromGiven` picks the first candidate without prompting (matches Emacs after issue #385 — the server already filters to formula-equal givens, so any candidate is a valid fill).  After each successful edit, `jumpToFirstHole` moves the cursor onto the first `?` inside the inserted text — mirror of Emacs's `deduce-lsp--apply-text-edit` cursor placement, so chained edits (refine → case split → induction) feel snappy.  Shared helpers (`ensureDeduceEditor`, `textDocumentPosition`, `applyLspWorkspaceEdit`, `applyCodeActionByTitle`, `applyCandidateEdit`, `applyFirstCandidateEdit`) sit at the bottom of `extension.js`; no new server endpoints needed — all five route to LSP requests that have existed since Phase 4.

- [x] **Step 31: `textDocument/completion` — in-buffer tab completion.** New `completions_at(path, content, pos, prelude)` in `lsp/query.py` returns a tuple of `CompletionCandidate(label, kind, detail, priority)` items: Deduce keywords (static list mirroring `deduce-mode--keywords`), constants (`true`/`false`), built-in types (`bool`/`type`), every top-level name reachable from the file (its own theorems / lemmas / postulates / defines / functions / unions / constructors / predicates / rules, plus the transitive walk through `Import.ast` — so `import Nat` surfaces `suc`, `zero`, `+`, `length`, ...), *and* in-scope local bindings at the cursor (proof labels from `assume`, term variables from `arbitrary` / pattern-bound names inside `switch` / `case`).  The env walk relies on `_completion_env_and_goal`: cursor on a literal `?` runs the original content with the hole targeted, otherwise scan backward over identifier characters, strip the partial identifier the user is typing, insert a synthetic `?` at the adjusted position, and read `IncompleteProof.env`/`.formula`.  Local-binding dedup uses the AST walker's `seen_labels` set (Deduce's `local` flag on `TermBinding` is module-visibility, not lexical scope, so it can't distinguish `arbitrary n:Nat` from imported decls; running the AST walker first ensures every top-level name is in `seen_labels` by the time the env walker contributes only lexically-local names).  Hole-aware priority boost: when an env+goal is available and a local label's formula equals or implies the goal (via `_matching_given_names`), the candidate is emitted with `priority=0`; everything else stays at the default `priority=1`.  `lsp/lsp_server.py` encodes `priority` as the LSP `sortText` field (`f"{priority}{label}"`) so matching labels float to the top of the picker — `Ctrl+Space` on a `?` surfaces proof terms that can plausibly close it first.  Kind→`CompletionItemKind` map: `keyword`→Keyword, `constant`→Constant, `type`/`union`→Class, `theorem`/`lemma`/`function`/`postulate`/`predicate`/`rule`→Function, `define`/`label`/`variable`→Variable, `constructor`→Constructor; returns `CompletionList(is_incomplete=False, ...)` so the client can cache and filter locally.  16-case pytest in `test/lsp/test_completion.py` locks the candidate set across own-file / transitive-import / parse-failure / local-binding / partial-identifier / hole-aware-priority shapes.  Editor wiring: both eglot and `vscode-languageclient` auto-register a completion handler as soon as the server advertises `completionProvider`; `M-TAB` / `C-M-i` in Emacs and `Ctrl+Space` in VS Code work out of the box.

- [x] **Step 32: LLM hole filling in VS Code.** `deduce.fillHole` command (`Ctrl+Alt+A`) ports `deduce-fill-hole.el` to VS Code by re-using the same `tools/claude_fill_hole` sidecar.  Flow: `deduce/holeContextAt` for the goal/givens/lemmas/fingerprint, snapshot the document version + hole text, spawn `<deduce.pythonPath> -m tools.claude_fill_hole` with cwd = `<deduce.deduceRoot>` and pipe a JSON request to stdin, await the JSON response on stdout (transport shared with Emacs and documented in `tools/claude_fill_hole/schema.py`).  Runs inside a `vscode.window.withProgress` notification so the user gets feedback + a cancel button (cancellation kills the subprocess).  Post-sidecar marker check: the hole's text must still be `?` AND a re-fetched `deduce/holeContextAt` must return the same fingerprint, else abort with a clear message and leave the buffer untouched.  Six new settings under `deduce.fillHole.*` (`backend`, `model`, `apiKeyEnv`, `baseUrl`, `maxAttempts`, `timeout`) mirror Emacs `defcustom`s; sensible defaults pick `claude-opus-4-7` + `ANTHROPIC_API_KEY` for `anthropic`, `gemma-4-31B-it` + `OPENAI_API_KEY` for `openai-compat`.  Multiple fill-hole requests may run concurrently, including in the same document; each request keeps its own range/fingerprint and stale-marker validation.  Multi-line proof responses get re-indented to the hole's column via `reindentProof`, a JS port of `deduce-fill-hole--reindent-proof`.  No new server-side work, no new sidecar work — the entire Python pipeline (anthropic / openai-compat backends, validate-proof loop, surrounding-source excerpt, prompt construction, cancel/timeout handling) is reused from `tools/claude_fill_hole/`, which Emacs has been driving since Phase 3 of the hole-fill plan.
