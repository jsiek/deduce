...
- [ ] **Step 27: VS Code syntax highlighting.** Hand-written TextMate grammar at `editor/vscode/syntaxes/deduce.tmLanguage.json`, wired via `contributes.grammars` in `package.json`.  Keyword categories mirror Emacs's `deduce-mode--keywords` / `--constants` / `--types` / `--warnings`; capitalized identifiers get `entity.name.type` so user-defined Unions and constructors get the same colorization Emacs's `font-lock-type-face` rule produces.  A follow-up step can replace the hand-written grammar with one generated from `gh_pages/scripts/keywords.py` once the web sandbox is ready to share the same source.

- [ ] **Step 28: VS Code LSP client.** Add `vscode-languageclient` to `editor/vscode/package.json`; in `extension.js`, instantiate a `LanguageClient` on the `deduce` language that spawns `<deduce.pythonPath> -m lsp.lsp_server` with cwd = `<deduce.deduceRoot>`.  Once running, VS Code gets the LSP server's existing features for free: live diagnostics, go-to-definition (F12 / Ctrl-click), document outline / breadcrumbs (`Cmd+Shift+O`), and the no-prompt code actions ("Refine hole", "Induction") via the lightbulb.  Reuses the existing `deduce.pythonPath` / `deduce.deduceRoot` settings and adds `deduce.noStdlib` to mirror Emacs's `deduce-lsp-prelude-disabled`.

- [ ] **Step 29: Goal-at-point command in VS Code.** Command palette entry `Deduce: Show goal at cursor` (bound to `Ctrl+Alt+G`) that issues the existing `deduce/goalAt` custom request and renders the goal in a dedicated `Deduce Goal` Output channel.  Mirror of Emacs's `C-c C-g` / `deduce-show-goal-at-point`.
